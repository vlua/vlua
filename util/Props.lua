...
local hasOwn, isObject, toRawType, hyphenate, capitalize, isPlainObject =
    Util.hasOwn,
    Util.isObject,
    Util.toRawType,
    Util.hyphenate,
...
---@param vm Component
local function validateProp(key, propOptions, propsData, vm)
    ...
    if (config.env ~= "production" and -- skip validation for weex recycle-list child component props
            not (__WEEX__ and isObject(value) and (value["@binding"] ~= nil))) then
...
local function getPropDefaultValue(vm, prop, key)
    ...
    if (vm and vm._options.propsData and vm._options.propsData[key] == nil and vm._props[key] ~= nil) then
        return vm._props[key]
    end
    -- call factory function for non-Function types
    -- a value is Function if its prototype is function even across different execution context
    return type(def) == "function" and getType(prop.type) ~= "Function" and def.call(vm) or def
end
...
local function assertProp(prop, name, value, vm, absent)
...
local function getInvalidTypeMessage(name, value, expectedTypes)
    local message =
        'Invalid prop: type check failed for prop "$thennameend".' +
            " Expected $thenexpectedTypes.map(capitalize).join(",
        ")end"

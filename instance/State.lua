...
initState = function(vm)
    ...
    if (opts.props) then initProps(vm, opts.props) end
    if (opts.methods) then initMethods(vm, opts.methods) end
    if (opts.data) then
        initData(vm)
    else
        vm._data = {}
    ...
    if (opts.computed) then initComputed(vm, opts.computed) end
...
initProps = function(vm, propsOptions)
    ...
    for key in pairs(propsOptions) do
        ...
        if (config.env ~= 'production') then
            ...
            if (isReservedAttribute(hyphenatedKey) or
                config.isReservedAttr(hyphenatedKey)) then
                warn(
                    '"_{hyphenatedKey}" is a reserved attribute and cannot be used as component prop.',
                    vm
                )
            end
    ...
    for key in pairs(data) do
        if (config.env ~= 'production') then
            if (methods and hasOwn(methods, key)) then
                warn(
                    'Method "_{key}" has already been defined as a data property.',
                    vm
                )
            end
        end
        if (props and hasOwn(props, key)) then
            if config.env ~= 'production' then
                warn(
                        'The data property "_{key}" is already declared as a prop. ' ..
...
initComputed = function(vm, computed)
    ...
    for key, userDef in pairs(computed) do
        ...
        if (config.env ~= 'production' and getter == nil) then
            warn(
            'Getter is missing for computed property "_{key}".',
            vm
            )
        end
...
defineComputed = function(
    ...
    if (type(userDef) == 'function') then
        getter = shouldCache
            and createComputedGetter(key)
            or createGetterInvoker(userDef)
        setter = noop
    else
        getter = userDef.get
            and ((shouldCache and userDef.cache ~= false)
            and createComputedGetter(key)
            or createGetterInvoker(userDef.get))
            or noop
...
initMethods = function(vm, methods)
    ...
    for key, method in pairs(methods) do
        if (config.env ~= 'production') then
            ...
            if (props and hasOwn(props, key)) then
                warn(
                    'Method "_{key}" has already been defined as a prop.',
                    vm
                )
            end

...
describe('api: watch', function()
  ...
  it('onTrack', function()
    ...
    , {onTrack=onTrack})
    lu.assertEquals(dummy, {1, true, {'foo', 'bar'}})
    expect(onTrack):toHaveBeenCalledTimes(3)
    lu.assertEquals(events, {{target=obj, type=TrackOpTypes.GET, key='foo'}, {target=obj, type=TrackOpTypes.HAS, key='bar'}, {target=obj, type=TrackOpTypes.ITERATE, key=ITERATE_KEY}})
  end
  )
  it('onTrigger', function()
    ...
    lu.assertEquals(events[0+1], {type=TriggerOpTypes.SET, key='foo', oldValue=1, newValue=2})
    obj.foo = nil
    expect(dummy):toBeUndefined()
    expect(onTrigger):toHaveBeenCalledTimes(2)
    lu.assertEquals(events[1+1], {type=TriggerOpTypes.DELETE, key='foo', oldValue=2})
  end
  )
end
)